...
public class BoardServiceImpl implements BoardService {
	 ...
	 * rollback처리한다. - rollbackFor를 저장하지 않으면 RuntimeException에러가 발생한 경우만 rollback
	 * 한다.
	 */
	public int insertBoard(Board b, List<BoardImg> imgList) {
		/*
		 * 서비스 로직 0. 게시슬 데이터 전처리(개행문자 처리 및 xss핸들링) 1. board테이블에 데이터 insert 2. 첨부파일이
		 * 존재하는경우 첨부파일 테이블에 insert 3. 1번, 2번과정에서 실패가 발생하는 경우 rollback
		 */
