...
// model에 들어가는 데이터 중, Session에 보관시킬 테이터를 설정하는 주석
public class MemberController {

	@Autowired // 의존성주입
	private MemberService mService; // = new MemberServiceImp1();
	
//	/*
//	 * Spring DI(Dependency Injection)
//	 * 	- 의존성 주입
//	 *  - 어플리케이션을 구성하는 객체를 개발자가 직접 생성하는게 아닌, 스프링이 생성한
//	 *  객체(bean)을 주입받아서 생성하는 방식
...
//	 * */
	 ...
	 *  - 클라이언트가 요청한 값을 대힌 반환하여 바인딩 해주는 역할은 argumentResolver
	 *  가 수행한다.
	 * */
	/*
	 * public String login(
	...
	// 매개변수의 이름과 "일치"하는 request의 파라미터을 추출하여 자동 바인딩.
	...
	/*public String login(String userId, String userPwd) {
	   ...
	   *  - 요청이 전달한 name속성과 일치하는 VO클래스 필드의 setter메서드를
	   *  호출하여 데이터를 바인딩
...
//		/*
//		 * 응갑데이터르 담을 수 있는 객체.(Model)
//		 * 1) Model
//		 *  - 포워딩 응답 뷰페이지에 전달하고자 하는 데이터를 맵형식으로 담을 수 있는 객체.
...
//		return mv;
//	}
	//@RequestMapps "/member/login", method=RequestMethod.POST)
	@PostMapping("/login")
	...
	public String logout(HttpSession session, SessionStatus status) {
		...
		status.setComplete(); // model로 session에 이관된 데이터를 삭제하는 메서드
...
//		
//		// Jackson-databind를 활용하여 , VO클래스, 컬렉션 테이터를 자동으로 
//		// JSON으로 반환해줄 수 있다.
//		return searchMember;
//	}
	 ...
	 *  - 컨트롤러에 예외처리 메서드를 1개 처리한 후, @ExceptionHandler어노테이션
	 *    추가.
	 *  - 2순위로 적용.
